...
export default function VendaDetalhePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-10">
      <Link href="/vendas" className="text-sm text-neutral-500 hover:underline">
        ← Vendas
      </Link>
      <PageHeader
          ...
          <tbody>
            {venda.itens.map((item) => (
              <tr key={item.id} className="border-b border-neutral-100 last:border-0">
                <td className="px-4 py-3 font-medium text-neutral-900">{item.produtoNome}</td>
                <td className="px-4 py-3 text-neutral-600">{item.quantidade}</td>
                <td className="px-4 py-3 text-neutral-600">{formatarMoeda(item.precoUnitario)}</td>
                <td className="px-4 py-3 text-neutral-600">{formatarMoeda(item.subtotal)}</td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr className="border-t border-neutral-200 bg-neutral-50">
              <td colSpan={3} className="px-4 py-3 text-right font-semibold text-neutral-700">
                Total
              </td>
              <td className="px-4 py-3 font-semibold text-neutral-900">{formatarMoeda(venda.valorTotal)}</td>
            </tr>
          </tfoot>
        </table>
      </Card>
