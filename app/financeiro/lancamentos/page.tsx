        ...
        <Card className="mb-6">
          <form onSubmit={criar} className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              {errosCampos.categoria && <p className="mt-1 text-xs text-red-600">{errosCampos.categoria}</p>}
            </div>
            <div>
            ...
            <tbody>
              {lancamentos.map((l) => (
                <tr key={l.id} className="border-b border-neutral-100 last:border-0">
                  <td className="px-4 py-3 text-neutral-600">{formatarData(l.dataLancamento)}</td>
                  <td className="px-4 py-3">
                    <Badge tone={l.tipo === "RECEITA" ? "success" : "danger"}>{l.tipo === "RECEITA" ? "Receita" : "Despesa"}</Badge>
                  </td>
                  <td className="px-4 py-3 font-medium text-neutral-900">{l.categoria}</td>
                  <td className="px-4 py-3 text-neutral-600">{l.descricao ?? "—"}</td>
                  <td className="px-4 py-3 text-neutral-600">{formatarMoeda(l.valor)}</td>
                  <td className="px-4 py-3 text-right">
                    {l.origem === "MANUAL" ? (
                      <button onClick={() => excluir(l)} className="text-red-600 hover:underline">
                        Excluir
                      </button>
                    ) : (
